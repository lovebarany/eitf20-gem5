...
int main()
{
    ...
    double first_float[size][size], second_float[size][size], multiply_float[size][size];
    ...
    for(int x=0; x<size; x++)
    {
        for(int y=0; y<size; y++)
        {
            first_float[x][y] = (2.37321238f * (double)x) + (-1.37123459f * (double)y);
	    first_float[x][y] = first_float[x][y] / 1.12395f;
            second_float[x][y] = (-0.64271237f * (double)x) + (3.5786147f * (double)y);
        }
    }
    //printf("Done!\n");
